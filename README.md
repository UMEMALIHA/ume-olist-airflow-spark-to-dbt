# Olist Data Pipeline: Migration from Airflow + Spark to dbt (POC)
...
1. Orchestration (Target): Apache Airflow (MWAA/EC2) triggers the pipeline steps  
...
### POC Execution Note
For the proof-of-concept on Windows, DuckDB is used as a lightweight local transformation engine to convert CSV → Parquet quickly. The curated Parquet layer and dbt logic remain Spark-compatible and can be deployed to EMR/Spark in production.
